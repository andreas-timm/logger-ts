...
function findNextVersion(output) {
    ...
    const nextVersion = /\bThe next release version is ([^\s]+)/.exec(
        plainOutput,
    );
    const releaseNoteVersion = /\bRelease note for version ([^:\s]+):/.exec(
        plainOutput,
    );

    return nextVersion?.[1] || releaseNoteVersion?.[1] || null;
}
...
async function main() {
    const semanticReleaseBin = require.resolve(
        "semantic-release/bin/semantic-release.js",
    );
    const result = spawnSync(
        process.execPath,
        [semanticReleaseBin, ...RELEASE_PREVIEW_ARGS],
        {
            cwd: process.cwd(),
            encoding: "utf8",
            env: process.env,
